...
attribute to you node resource specification, e.g., the following node
...
.. warning::
   ...
   is copied to permanent storage such as ``$VSC_DATA`` before you job ends.

The path to the root if this file system is given by the environment variable
``$VSC_SCRATCH_JOB`` and is only defined in the process that runs your job script
on the compute node.

.. note::
   BeeGFS is a parallel file system, hence MPI application that use MPI-I/O,
   or application that rely on parallel HDF5 may benefit greatly from using
   it.
